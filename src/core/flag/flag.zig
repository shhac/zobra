...
// The apply-loop dispatcher (applyTokensWith, applyLong/Short/Negated,
// LookupLong/ShortFn) lives in apply.zig — re-exported above so
// existing callers keep working. The type-coercion / storage-binding
// helpers (setStored, bind*, free*, failCoerce) live in bind.zig.
// Tests for the apply path are in test/flag/flagset.zig.
